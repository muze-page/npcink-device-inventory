...
- `POST /wp-json/npcink/v1/device-observations`：软件端上传采集事实。
- `GET|POST /wp-json/npcink/v1/assets`：管理资产列表与新增资产。
- `GET|PATCH|DELETE /wp-json/npcink/v1/assets/{uuid}`：资产详情、更新、软删除。
- `GET|POST /wp-json/npcink/v1/assets/{uuid}/identities`：资产身份。
- `GET /wp-json/npcink/v1/assets/{uuid}/observations`：资产采集快照。
- `GET|POST /wp-json/npcink/v1/assets/{uuid}/events`：资产事件时间线。
- `GET|PATCH /wp-json/npcink/v1/settings`：插件设置。
- `POST /wp-json/npcink/v1/client-tokens`：创建软件端授权码。
- `PATCH|DELETE /wp-json/npcink/v1/client-tokens/{id}`：启停或删除软件端授权码。
- `GET /wp-json/npcink/v1/client-tokens/{id}/package-config`：复制客户端预设打包配置。
...
- 设备上传使用 `/wp-json/npcink/v1/device-observations`，必须使用后台生成的完整授权码。客户端会用 timestamp、nonce、body hash 和 HMAC 签名提交。
- 软件端可填写站点首页、`/wp-json`、`/wp-json/npcink/v1` 或完整上传 endpoint，客户端会自动归一化到 v3 上传接口。
