...
The WordPress package includes the built files and the corresponding React/TypeScript source. Admin app source is in `vite-admin/src`.
...
在 WordPress 中管理设备资产：后台台账、签名客户端上报、变更记录和导入导出。
...
不允许。设备上传需要客户端授权码和 HMAC 签名。管理端接口需要具备 `manage_options` 权限的 WordPress 用户。
...
插件包包含构建产物和对应的 React/TypeScript 源码。后台应用源码在 `vite-admin/src`。
