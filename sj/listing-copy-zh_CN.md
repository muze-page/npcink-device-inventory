...
Npcink Device Manage 是一个设备资产管理插件，适合希望把硬件库存和设备台账保存在 WordPress 内的团队使用。
...
1. 将 `npcink-device-manage` 文件夹上传到 `/wp-content/plugins/`。
2. 在 WordPress 插件页面启用插件。
3. 打开 Plugins > Device Asset Management。
...
插件包包含构建产物和对应的 React/TypeScript 源码。后台应用源码在 `vite-admin/src`，公开查询应用源码在 `vite-search/src`，构建配置位于各自的 `package.json` 和 `vite.config.ts`。

## 隐私

Npcink Device Manage 将设备资产数据存储在本地 WordPress 数据库中。根据站点管理员的配置和使用方式，数据可能包括设备名称、分配用户或位置、部门、状态、IP 地址、硬件标识、硬件详情和变更历史。
