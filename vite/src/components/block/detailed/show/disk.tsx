...
const App: React.FC<Props> = ({ data }) => {
  const formattedData = (item: ComputerDevice) => {
    const arr = [
      ...
      { key: "3", label: "名称", value: item.name },
      { key: "4", label: "供应商", value: item.vendor },
      ...
      { key: "15", label: "接口类型", value: item.interfaceType },
    ...
    <>
      {data.map((item, index) => {
        return (
          <>
            <div key={index}>
              <p className="font-black my-2 text-xl">硬盘 - {index+1}</p>
